<template>
  <v-container>
    <v-row justify="center">
      <v-col>
        <v-card>
          <v-card-title class="text-center text-h5" style="font-family: 'Playfair Display', serif; font-weight: 900;">
            Dining Reservation
          </v-card-title>
          <v-card-text>
              ...
              <v-card class="weatherview" style="margin-top: 10px; display: none;">
                <v-card-text>
                  ...
                  <v-row style="padding-left: 50px; padding-top: 10px; padding-bottom: 10px;">
                    <v-btn type="submit" class="text-center custom-btn">예약하기</v-btn>
                  </v-row>
                </v-card-text>
              </v-card>
            </v-form>
...
export default {
  ...
  methods: {
    ...
    handleDateChange(date) {
      // 날짜 형식을 YYYY-MM-DD로 변환 (필요에 따라 형식을 조정할 수 있습니다)
      this.selectedDate = date ? date.toISOString().split('T')[0] : null;
    },
    async diningReservation() {
      try {
        // 날짜와 시간을 합쳐서 reservationDateTime을 생성
        const reservationDateTime = `${this.selectedDate} ${this.timeInput}`;
